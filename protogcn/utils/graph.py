import numpy as np
import torch


def k_adjacency(A, k, with_self=False, self_factor=1):
    if isinstance(A, torch.Tensor):
        A = A.data.cpu().numpy()
    assert isinstance(A, np.ndarray)

    I = np.eye(len(A), dtype=A.dtype)
    if k==0:
        return I
    

    # (A+I)^k
    Ak = np.minimum(np.linalg.matrix_power(A+I, k), 1) - np.minimum(np.linalg.matrix_power(A+I, k-1), 1)
    if with_self:
        Ak += (self_factor * I)
    
    return Ak


def edge2adjacency(link, num_node):
    A = np.zeros((num_node, num_node))
    for i, j in link:
        A[j, i] = 1
    return A


def normalize_digraph(A, dim=0):
    degrees = np.sum(A, dim)
    h, w = A.shape
    # D^-1 : matrix for normalize to divide value by degree
    D = np.zeros((w, w))

    for i in range(w):
        if degrees[i] > 0:
            D[i, i] = degrees[i] ** (-1)

    # AxD^-1
    AD = np.dot(A, D)
    return AD

def get_hop_distance(num_node, edge, max_hop=1):
    """get shortest distance(hop distance) between all node pairs"""
    A = np.eye(num_node)

    for i, j in edge:
        A[i, j] = 1
        A[j, i] = 1

    hop_distance = np.zeros((num_node, num_node)) + np.inf 
    transfer_matrix = [
        np.linalg.matrix_power(A, d) for d in range(max_hop + 1)
    ]
    arrive_matrix = (np.stack(transfer_matrix)> 0)
    for d in range(max_hop, -1, -1):
        # boolean indexing : select only True 
        hop_distance[arrive_matrix[d]] = d

    return hop_distance


class Graph :
    def __init__(self, 
                 layout='coco', 
                 mode='spatial', 
                 max_hop=1, 
                 nx_node=1,
                 num_filter=3,
                 init_std=0.02,
                 init_off=0.04):
        
        self.max_hop = max_hop
        self.layout = layout
        self.mode = mode
        self.num_filter = num_filter
        self.init_std = init_std
        self.init_off = init_off
        self.nx_node = nx_node

        assert nx_node==1 or mode=='random', "nx_node can be > 1 only if mode is 'random'"
        assert layout in ['nturgb+d', 'openpose', 'openpose_new', 'coco', 'coco_new']

        self.get_layout(layout)
        self.hop_distance = get_hop_distance(self.num_node, self.inward, self.max_hop)

        assert hasattr(self, mode), f'Do Not Exist This Mode: {mode}'
        self.A = getattr(self, mode)()

    def __str__(self):
        return self.A
    
    def get_layout(self, layout):
        if layout == 'openpose':
            self.num_node = 18
            self.inward = [
                (4, 3), (3, 2), (7, 6), (6, 5), (13, 12), (12, 11), (10, 9),
                (9, 8), (11, 5), (8, 2), (5, 1), (2, 1), (0, 1), (15, 0),
                (14, 0), (17, 15), (16, 14)
            ]
            self.center = 1
        elif layout == 'openpose_new':
            self.num_node = 20
            self.inward = [
                (4, 3), (3, 2), (7, 6), (6, 5), (13, 12), (12, 11), (10, 9),
                (9, 8), (11, 18), (8, 18), (5, 1), (2, 1), (0, 1), (15, 0),
                (14, 0), (17, 15), (16, 14), (18, 19), (19, 1)
            ]
            self.center = 1
        elif layout == 'nturgb+d':
            self.num_node = 25
            neighbor_base = [
                (1, 2), (2, 21), (3, 21), (4, 3), (5, 21), (6, 5), (7, 6),
                (8, 7), (9, 21), (10, 9), (11, 10), (12, 11), (13, 1),
                (14, 13), (15, 14), (16, 15), (17, 1), (18, 17), (19, 18),
                (20, 19), (22, 8), (23, 8), (24, 12), (25, 12)
            ]
            self.inward = [(i - 1, j - 1) for (i, j) in neighbor_base]
            self.center = 21 - 1
        elif layout == 'coco':
            self.num_node = 17
            self.inward = [
                (15, 13), (13, 11), (16, 14), (14, 12), (11, 5), (12, 6),
                (9, 7), (7, 5), (10, 8), (8, 6), (5, 0), (6, 0),
                (1, 0), (3, 1), (2, 0), (4, 2)
            ]
            self.center = 0
        elif layout == 'coco_new':
            self.num_node = 20
            self.inward = [
                (15, 13), (13, 11), (16, 14), (14, 12), (11, 17), (12, 17),
                (9, 7), (7, 5), (10, 8), (8, 6), (5, 19), (6, 19),
                (1, 0), (3, 1), (2, 0), (4, 2), (0, 19), (17, 18), (18, 19)
            ]
            self.center = 19
        else:
            raise ValueError(f'Do Not Exist This Layout: {layout}')
        
        self.self_link = [(i, i) for i in range(self.num_node)]
        self.outward = [(j, i) for i, j in self.inward]
        self.neighbor = self.inward + self.outward
    
    def stgcn_spatial(self):
        """
        Spatial configuration partitioning

        return : A 
                A[0] : root 
                A[1] : centrifugal
                A[2] : centripetal
        """

        adj = np.zeros((self.num_node, self.num_node))
        adj[self.hop_distance <= self.max_hop] = 1
        normalized_adj = normalize_digraph(adj)
        hop_dis = self.hop_distance
        center = self.center

        A = []
        for hop in range(self.max_hop + 1):
            # Centrifugal 
            a_close = np.zeros((self.num_node, self.num_node))
            # Centripetal 
            a_further = np.zeros((self.num_node, self.num_node))
            for i in range(self.num_node):
                for j in range(self.num_node):
                    if hop_dis[j, i] == hop:
                        # neighbor j distance > root i distance -> Centrifugal 
                        if hop_dis[j, center] >= hop_dis[i, center]:
                            a_close[j, i] = normalized_adj[j, i]
                        else:
                            a_further[j, i] = normalized_adj[j, i]
            A.append(a_close)
            if hop > 0:
                A.append(a_further)
        return np.stack(A)
    
    def spatial(self):
        I = edge2adjacency(self.self_link, self.num_node)
        In = normalize_digraph(edge2adjacency(self.inward, self.num_node))
        Out = normalize_digraph(edge2adjacency(self.outward, self.num_node))
        A = np.stack((I, In, Out))
        return A
    
    def binary_adj(self):
        A = edge2adjacency(self.inward + self.outward, self.num_node)
        return A[None] # (1, num_node, num_node)
    
    def random(self):
        num_node = self.num_node *self.nx_node
        return np.random.randn(self.num_filter, num_node, num_node) * self.init_std + self.init_off


