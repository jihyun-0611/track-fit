...
class PreNormalize3D:
    """
    PreNormalize for NTURGB+D 3D keypoint (x, y, z).
    """
    ...
    def angle_between(self, v1, v2):
        """Returns the angle in radians betweens vectors 'v1' and 'v2'. """
...
class Part_Drop:
    """Drop the left or right limbs of the skeleton (for NTU RGB+D)"""
...
class JointToBone: 
    def __init__(self, dataset='nturgb+d', target='keypoint'):
        ...
        if self.dataset not in ['nturgb+d', 'openpose', 'openpose_new', 'coco', 'coco_new']:
            raise ValueError(
                f'The dataset type {self.dataset} is not supposed'
            )
        if self.dataset == 'nturgb+d':
...
class MergeSkeFeat:
    def __init__(self, feat_list=['keypoint'], target='keypoint', axis=-1):
        """Merge different feats (ndarray) by concatenate them in the last"""
...
class DecompressPose:
    ...
    Args:
        ...
        max_person (int): The max number of persons in a frame, we keep skeletons with scores from high to low.
            Default: 10.
    """
    def __init__(self, squeeze=True, max_person=10):
